 ...
 */
const getTgBroadcastCount = async (req, res) => {
    try {
        const filter = buildFilter(req.query);
        const count = await telegramUserSchema_1.default.countDocuments(filter);
...
function buildFilter(q) {
    ...
    if (q.divisionId)
        filter.divisionId = q.divisionId;
    if (q.districtId)
        filter.districtId = q.districtId;
    if (q.thanaId)
        filter.thanaId = q.thanaId;
    ...
    if (q.neverDonated === "true") {
        ...
        const dateFilter = {};
        if (q.lastDonationFrom)
            dateFilter.$gte = new Date(q.lastDonationFrom);
        if (q.lastDonationTo)
            dateFilter.$lte = new Date(q.lastDonationTo);
    ...
    if (q.registeredFrom || q.registeredTo) {
        const regFilter = {};
        if (q.registeredFrom)
            regFilter.$gte = new Date(q.registeredFrom);
        if (q.registeredTo)
            regFilter.$lte = new Date(q.registeredTo);
        filter.createdAt = regFilter;
    }
    return filter;
}
exports.default = getTgBroadcastCount;
