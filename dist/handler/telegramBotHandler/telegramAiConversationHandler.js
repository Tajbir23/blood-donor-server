...
function isFollowUp(text) {
    const t = text.trim();
    if (t.length < 25 && /^(আরো|আরও|বিস্তারিত|বিস্তার|তাহলে|কেন|কীভাবে|কিভাবে|তারপর|আর কি|এরপর|ok|okay|হ্যাঁ|yes|more|else|further|explain|got it|তারপর কি|আর বলো|বলো|কি করব|এখন কি করব)/i.test(t))
        return true;
    ...
    try {
        ...
        if (prediction.intent === "BLOOD_INFO") {
            ...
            const queryText = (ctxState.lastIntent === "BLOOD_INFO" && ctxState.lastFaqQuery && isFollowUp(text))
                ...
                : text;
            const faq = (0, faqKnowledgeBase_1.findFaqAnswer)(queryText) || (queryText !== text ? (0, faqKnowledgeBase_1.findFaqAnswer)(text) : null);
            if (faq) {
                ...
                if (ctxState.lastIntent === "BLOOD_INFO" && isFollowUp(text)) {
                    await (0, sendMessageToTgUser_1.sendTgInlineKeyboard)(chatId, "🩸 এ বিষয়ে আরো কী জানতে চান? নিচে বিষয় বেছে নিন:", [["রক্তদানের বয়স", "কতদিন পর পর দেওয়া যায়"], ["ট্যাটু ও পিয়ার্সিং", "রক্তদানের পর খাবার"], ["গর্ভাবস্থা ও রক্তদান", "প্রথমবার রক্তদান"]]);
                }
                else {
        ...
        if (prediction.intent === "GREET") {
            clearTgAiState(chatId);
            // Smart greeting — pick response based on what the user actually typed
            const lowerText = text.toLowerCase().trim();
            let randomGreeting;
            if (/assalamu|assalam|salam|আস্সালামু|আস্সালাম|সালাম/.test(lowerText)) {
                // Islamic greeting → Islamic reply
                const opts = [
                    "ওয়া আলাইকুমুস সালাম ওয়া রাহমাতুল্লাহ! 😊",
                    "ওয়া আলাইকুমাস সালাম! ভালো আছি, আলহামদুলিল্লাহ। 😊",
                ];
                randomGreeting = opts[Math.floor(Math.random() * opts.length)];
            }
            else if (/walaikum|ওয়ালাইকুম/.test(lowerText)) {
                randomGreeting = "আলহামদুলিল্লাহ, ভালো আছি! আপনিও ভালো থাকুন। 😊";
            }
            else if (/good morning|সুপ্রভাত/.test(lowerText)) {
                randomGreeting = "শুভ সকাল! 🌅 আজকে কীভাবে সাহায্য করতে পারি?";
            }
            else if (/good evening|good night|শুভ রাত|শুভ সন্ধ্যা/.test(lowerText)) {
                randomGreeting = "শুভ সন্ধ্যা! 🌙 আজকে কীভাবে সাহায্য করতে পারি?";
            }
            else if (/কেমন আছ|কেমন আছেন|কি খবর|কী খবর/.test(lowerText)) {
                const opts = [
                    "আলহামদুলিল্লাহ, ভালো আছি! আপনি কেমন আছেন? 😊",
                    "ভালো আছি, ধন্যবাদ জিজ্ঞেস করার জন্য! 😊",
                ];
                randomGreeting = opts[Math.floor(Math.random() * opts.length)];
            }
            else if (/how are you|how r u/.test(lowerText)) {
                randomGreeting = "I'm doing great, thanks for asking! 😊";
            }
            else if (/namaste|নমস্কার|আদাব/.test(lowerText)) {
                randomGreeting = "নমস্কার! 🙏 কীভাবে সাহায্য করতে পারি?";
            }
            else {
                // Generic hi/hello/hey/হ্যালো etc.
                const opts = [
                    "হ্যালো! আপনার সাথে কথা বলতে পেরে ভালো লাগছে! 😊",
                    "হেই! কীভাবে সাহায্য করতে পারি? 😊",
                    "আন্তরিক স্বাগতম! 😊",
                ];
                randomGreeting = opts[Math.floor(Math.random() * opts.length)];
            }
            await (0, sendMessageToTgUser_1.sendTgInlineKeyboard)(chatId, `${randomGreeting}\n\n` +
                ...
                `নিচের মেনু থেকে বেছে নিন 👇`, [["🔍 রক্তদাতা খুঁজুন", "📝 ডোনার নিবন্ধন"], ["❓ সাহায্য", "🌐 ওয়েবসাইট"]]);
        ...
        if (prediction.intent === "HELP") {
            clearTgAiState(chatId);
            await (0, sendMessageToTgUser_1.sendTgInlineKeyboard)(chatId, "🩸 <b>LifeDrop Bot যা করতে পারে:</b>\n\n" +
                "🔍 <b>রক্তদাতা খোঁজা</b> — এলাকা ও রক্তের গ্রুপ দিলেই কাছের ডোনার দেখাবে\n" +
                "   <i>\"A+ রক্ত দরকার ঢাকায়\"</i>\n\n" +
                "📝 <b>ডোনার নিবন্ধন</b> — এক-বারে ৫টি ধাপে নিবন্ধন করুন\n" +
                "   <i>\"আমি রক্তদাতা হতে চাই\"</i>\n\n" +
                "🔄 <b>প্রোফাইল আপডেট</b> — নাম, ফোন, এলাকা, শেষ দান পরিবর্তন করুন\n\n" +
                "❓ <b>রক্তদান সম্পর্কে যেকোনো প্রশ্ন</b> সরাসরি লিখুন:\n" +
                "   <i>\"কতদিন পর রক্ত দেওয়া যায়?\"</i>\n" +
                "   <i>\"ট্যাটু করলে কি রক্ত দেওয়া যায়?\"</i>\n" +
                "   <i>\"প্রথমবার রক্ত দেওয়ার আগে কী করব?\"</i>\n\n" +
                "নিচের বোতামে ক্লিক করুন 👇", [["🔍 রক্তদাতা খুঁজুন", "📝 ডোনার নিবন্ধন"], ["🔄 প্রোফাইল আপডেট", "📅 শেষ দান আপডেট"], ["🌐 ওয়েবসাইট"]]);
        ...
        // Smart UNKNOWN: show actionable menu instead of just a website link
        await (0, sendMessageToTgUser_1.sendTgInlineKeyboard)(chatId, "🤔 আপনার কথাটি পুরোপুরি বুঝতে পারিনি।\n\n" +
            "একটু সহজ করে লিখলে ভালো হয়, যেমন:\n" +
            "• <i>\"A+ রক্ত দরকার ঢাকায়\"</i>\n" +
            "• <i>\"আমি ডোনার হতে চাই\"</i>\n" +
            "• <i>\"কতদিন পর রক্ত দেওয়া যায়?\"</i>\n\n" +
            "অথবা নিচের মেনু থেকে বেছে নিন 👇", [["🔍 রক্তদাতা খুঁজুন", "📝 ডোনার নিবন্ধন"], ["🔄 প্রোফাইল আপডেট", "📅 শেষ দান আপডেট"], ["❓ সাহায্য", "🌐 ওয়েবসাইট"]]);
        // Keep the website button too for direct access
        await (0, sendMessageToTgUser_1.sendTgUrlButton)(chatId, "সরাসরি ওয়েবসাইটে গিয়েও রক্তদাতা খুঁজতে পারেন:", "রক্তদাতা খুঁজুন →", `${process.env.FRONTEND_URL}/blood-donation`);
        return true;
    }
    catch (err) {
