...
const detectVpn = (req, res, next) => {
    ...
    const rawIp = req.ip || ((_a = req.socket) === null || _a === void 0 ? void 0 : _a.remoteAddress);
    if (!rawIp) {
        res.status(403).json({
            success: false,
    ...
    try {
        ...
        if (detectedVpnHeaders.length > 0) {
            console.warn(`[VPN-Detect] BLOCKED — BD IP with VPN headers | IP: ${cleanIp} | headers: [${detectedVpnHeaders.join(', ')}] | via: ${viaHeader || 'none'}`);
            res.status(403).json({
                success: false,
